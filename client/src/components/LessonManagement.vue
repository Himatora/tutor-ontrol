          ...
          <div class="form-group m-3">
            ...
            <div class="mb-3">
              ...
              <div class="input-group mb">
                <select v-model="newLesson.topic" class="form-select" required>
...
export default {
  ...
  setup(props) {
    ...
    // Проверяем валидность формы урока
    ...
    const fetchTopics = async () => {
      try {
        ...
        if (topics.value.length > 0 && !newLesson.value.topic) {
          newLesson.value.topic = topics.value[0].id; // Исправлено на newLesson.value.topic
        }
      } catch (error) {
    ...
    const addLesson = async () => {
      ...
      try {
        ...
        console.log('Отправляемые данные урока:', lessonData);
        await axios.post('/api/lessons/', lessonData, {
          headers: { 'Content-Type': 'application/json' },
        });
        newLesson.value = {
          student: props.studentId,
          lesson_type: lessonTypes.value.length > 0 ? lessonTypes.value[0].id : null,
          topic: topics.value.length > 0 ? topics.value[0].id : null,
          comment: '',
        };
      } catch (error) {
